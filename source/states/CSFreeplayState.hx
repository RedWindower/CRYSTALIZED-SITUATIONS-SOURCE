...
class CSFreeplayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		magenta = new FlxSprite(-80).loadGraphic(Paths.image('menuBLACKDesat'));
		...
		var guide:FlxText = new FlxText(12, FlxG.height - 44, 0, "HEY! " + guide, 12);
				...
				else
				{
					...
					FlxFlicker.flicker(menuItems.members[curSelected], 1, 0.06, false, false, function(flick:FlxFlicker)
					{
						switch (optionShit[curSelected])
						{
							...
							case 'memories':
								PlayState.SONG = backend.Song.loadFromJson('memories-hard', 'memories'); 
								LoadingState.loadAndSwitchState(new PlayState());
							case 'realvancy':
								PlayState.SONG = backend.Song.loadFromJson('realvancy-hard', 'realvancy'); 
								LoadingState.loadAndSwitchState(new PlayState());
							case 'rewind':
								PlayState.SONG = backend.Song.loadFromJson('rewind-hard', 'rewind'); 
								LoadingState.loadAndSwitchState(new PlayState());
						}
					});
